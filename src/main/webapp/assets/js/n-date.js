...
(function ($) {   
    ...
    $.fn.zydate = function (options,Ycallback,Ncallback) {   
        //插件默认选项
        var that = $(this);
        ...
        var HourScroll=null,MinuteScroll=null,SecondScroll=null,DataScroll=null;
        
        $.fn.zydate.defaultOptions = {
            beginyear:1950,                 //日期--年--份开始
            endyear:initY,                   //日期--年--份结束
            ...
            theme:"date",                    //控件样式（1：日期，2：日期+时间）,diydata ==自定义选项
            mode:null,                       //操作模式（滑动模式）
            event:"focus",                    //打开日期插件默认方式为点击后后弹出日期
            show:true,
        }
        //用户选项覆盖插件默认选项   
        var opts = $.extend( true, {}, $.fn.zydate.defaultOptions, options );
        if(opts.theme === "datetime"){datetime = true;}
        if(opts.theme === "diydata"){diydata = true;}
        if(!opts.show){
            that.unbind('click');
        }
        ...
        function refreshData(){
            DataScroll.refresh();
            DataScroll.scrollTo(0, Math.floor(opts.diydata.length/2)*40, 100, true);
        }
	    function resetIndex(){
            indexY=1;
            indexM=1;
        ...
        function bindButton(){
            ...
            $("#dateconfirm").unbind('click').click(function () {	
               ...
               if(diydata){
                     datestr = $("#diydatawrapper ul li:eq("+indexData+")").html();
                     console.log(datestr);
                }
        ...
        function showdiydata(){
            init_iScroll_diydata();
            // addTimeStyle();
            $("#datescroll").hide();
            $("#datescroll_data").show(); 
            $("#diydatawrapper ul").html(createDIYDATA_UL());
        }
        ...
        function init_iScroll_diydata(){
            DataScroll = new iScroll("diydatawrapper",{snap:"li",vScrollbar:false,
                onScrollEnd:function () {
                    indexData = Math.round((this.y/40)*(-1))+1;
                    // Math.floor(opts.diydata.length/2)
                    DataScroll.refresh();
            }})
        } 
                ...
                '<div id="datePage" class="page">'+
                    '<section>'+
                        ...
                        '<div id="datescroll_datetime">'+
                            ...
                            '</div>'+
                        '</div>'+
                        '<div id="datescroll_data">'+
                            '<div id="diydatawrapper">'+
                                '<ul></ul>'+
                            '</div>'+
                        '</div>'+
                    '</section>'+
        ...
        function addTimeStyle(){
            $("#datePage").css({"height":"330px"});
            $("#yearwrapper").css("position","absolute");
            $("#yearwrapper").css("bottom","140px");
            $("#monthwrapper").css("position","absolute");
            $("#monthwrapper").css("bottom","140px");
            $("#daywrapper").css("position","absolute");
            $("#daywrapper").css("bottom","140px");
        }
        //创建 --年-- 列表
        ...
        //创建 --自定义-- 列表
        function createDIYDATA_UL(){
            $("#diydatawrapper ul").html("");
            var str="<li>&nbsp;</li>";
                for(var i=0;i<=opts.diydata.length-1;i++){
                str+='<li>'+opts.diydata[i]+'</li>'
            }
            return str+"<li>&nbsp;</li>";;                     
        }
    }
